const SHEET_ID = "1M7VU5o3mon-O35BkFiXmrHmeN2ZOHSoU5RUKQa9BY3U";
const GARDIEN_GID = "1991162609";
const GVIZ_URL = `https://docs.google.com/spreadsheets/d/${SHEET_ID}/gviz/tq?tqx=out:json&gid=${GARDIEN_GID}`;

export type KeeperTechnique = {
  id: string;
  nameFr: string;
  nameEn: string;
  nameJp: string;
...
export async function fetchKeeperTechniques(): Promise<KeeperTechnique[]> {
  ...
  if (!response.ok) {
    throw new Error("Impossible de récupérer la doc Gardien");
  }
  ...
  return rows
    .map((row, index) => {
      ...
      if (!nameFr || normalizeLabel(nameFr) === "nom / name francais") {
        return null;
      }

      const technique: KeeperTechnique = {
    ...
    .filter((technique): technique is KeeperTechnique => Boolean(technique));
}
...
function pickLocation(cells: string[] | undefined, startIndex: number) {
  ...
  const safeStart = startIndex >= 0 ? startIndex : 5;
