...
type TirsTechniqueCardProps = {
  technique: TirsTechnique;
  onSelect?: () => void;
  isFocused?: boolean;
};

export default function TirsTechniqueCard({
  technique,
  onSelect,
  isFocused = false,
}: TirsTechniqueCardProps) {
  ...
  const hasShotBlock =
    technique.shotBlock !== "—" &&
    technique.shotBlock.toLowerCase().includes("oui");
      ...
      <header className="relative z-10 flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-col items-end gap-1">
          <div className="flex flex-wrap justify-end gap-2">
            ...
            {technique.shotType !== "—" && (
              <span className="rounded-full border border-violet-500/20 bg-violet-500/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-wide text-violet-700 dark:border-violet-500/30 dark:bg-violet-500/20 dark:text-violet-400">
                {technique.shotType}
              </span>
            )}
          </div>
        ...
        <div className="rounded-2xl border border-dashed border-border bg-muted/50 px-4 py-3 text-sm text-foreground">
          ...
          {technique.price !== "—" && (
            <p className="mt-2 text-xs text-muted-foreground">
              Prix indicatif :{" "}
              <span className="font-semibold">{technique.price}</span>
            </p>
          )}
