...
import type { OffensiveTechnique } from "@/lib/doc-offensive";
...
type OffensiveTechniqueCardProps = {
  technique: OffensiveTechnique;
  onSelect?: () => void;
  isFocused?: boolean;
};

export default function OffensiveTechniqueCard({
  technique,
  onSelect,
  isFocused = false,
}: OffensiveTechniqueCardProps) {
    ...
    >
      ...
      <header className="relative z-10 flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-col items-end gap-1">
          ...
          <p className="text-3xl font-black text-slate-900">{technique.off}</p>
        </div>
      </header>

      <div className="relative z-10 space-y-3">
        {hasShotBlock && (
          <div className="rounded-2xl border border-emerald-200 bg-emerald-50/80 px-4 py-2 text-xs font-semibold text-emerald-700">
            ✓ Blocage de tir
          </div>
        )}
